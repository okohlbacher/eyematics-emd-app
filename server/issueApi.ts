...
/**
 * Production Express middleware handler for GET/POST /api/issues.
...
export function issueApiHandler(
  ...
  if (req.method === 'POST' && (req as { url?: string }).url === '/api/issues') {
    if (!expressReq.auth) {
  ...
  if (req.method === 'GET' && (req as { url?: string }).url === '/api/issues/export') {
  ...
  if (req.method === 'GET' && (req as { url?: string }).url === '/api/issues') {
    if (!expressReq.auth) {
      sendError(res, 401, 'Authentication required');
      return;
    }
